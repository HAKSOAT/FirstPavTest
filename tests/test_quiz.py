...
class QuizSolve(unittest.TestCase):
    ...
    def test_solve_unavailable_quiz(self):
        data = {"answers": ["England", "England", "Spain", "Italy",
                            ...
                            "Swaziland"]}
        response = self.client.post("/quiz/2/solve", headers=self.headers,
    ...
    def test_quiz_solve_with_credentials(self):
        data = {"answers": ["England", "England", "Spain", "Italy",
                            ...
                            "Netherlands", "France", "Portugal", "Ghana",
                            "Swaziland"]}
        response = self.client.post("/quiz/1/solve", headers=self.headers,
                                    data=json.dumps(data), content_type='application/json')
        with self.app.app_context():
    ...
    def test_quiz_solve_without_credentials(self):
        data = {"answers": ["England", "England", "Spain", "Italy",
                            ...
                            "Netherlands", "France", "Portugal", "Ghana",
                            "Sweden"]}
        response = self.client.post("/quiz/1/solve", data=json.dumps(data), content_type='application/json')
    ...
    def test_quiz_solve_with_invalid_number_of_answers(self):
        ...
        response = self.client.post("/quiz/1/solve", headers=self.headers,
                                    data=json.dumps(data), content_type='application/json')
        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json["message"], "13 answers are not available")
